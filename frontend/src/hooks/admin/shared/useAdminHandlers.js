...
// ✅ 목록 새로고침 핸들러
export const refreshList = async (getList, setItems, setDisplayedItemCount, setShowItems, setCurrentPage, onError) => {
  try {
    ...
    setItems(items);
    setDisplayedItemCount(items.length);
    setShowItems(true);
    setCurrentPage(1);
  } catch (error) {
...
// ✅ 상세 페이지 이동
export const navigateToEditPage = (navigate, page, itemId) => {
  navigateToAdminPage(navigate, page, itemId);
};