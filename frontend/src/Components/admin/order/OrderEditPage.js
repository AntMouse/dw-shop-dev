...
function OrderEditPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // 주문 정보를 가져옴
        const orderResponse = await axios.get(`${API_BASE_URL}/api/admin/orders/${id}`);
        const orderData = orderResponse.data;
        setOrder(orderData);
        setEditOrderData({ status: orderData.status });

        // 회원 정보를 가져옴
        const memberResponse = await axios.get(`${API_BASE_URL}/api/members/${orderData.memberId}`);
        const memberData = memberResponse.data;
        setMember(memberData);

        // 주문 상태를 가져옴
        const statusesResponse = await axios.get(`${API_BASE_URL}/api/order-statuses`);
  ...
  const handleSaveClick = () => {
    axios.put(`${API_BASE_URL}/api/admin/orders/${id}/status`, { status: editOrderData.status })
      .then(() => {
