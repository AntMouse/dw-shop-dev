// components/admin/order/orderHandlers.js
...
export const fetchOrderStatuses = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/order-statuses`);
...
export const deleteOrders = async (orderIds) => {
  try {
    const response = await axios.delete(`${API_BASE_URL}/api/admin/orders`, { data: orderIds });
...
export const updateOrderStatus = async (orderIds, status) => {
  try {
    const response = await axios.put(`${API_BASE_URL}/api/admin/orders/status`, { orderIds, status });
    return response.data;
  } catch (error) {
