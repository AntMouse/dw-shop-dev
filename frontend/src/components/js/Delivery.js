...
function ShippingAddressForm() {
    ...
    const handleSave = async () => {
        const token = localStorage.getItem('token');
        try {
            await axios.put(`${API_BASE_URL}/api/addresses/update/${addressList[editIndex].id}`, editedAddress, {
    ...
    const handleDelete = async (id) => {
        const token = localStorage.getItem('token');
        try {
            await axios.delete(`${API_BASE_URL}/api/addresses/delete/${id}`, {
                headers: { 'Authorization': token }
            });
            fetchAddresses();
        } catch (error) {
    ...
    const handleAdd = async () => {
        const token = localStorage.getItem('token');
        try {
            await axios.post(`${API_BASE_URL}/api/addresses/add`, newAddress, {
                headers: { 'Authorization': token }
            });
            fetchAddresses();
        } catch (error) {
