...
import com.example.DWShopProject.order.dto.AOrderDto;
import com.example.DWShopProject.order.dto.AOrderItemDto;
...
import com.example.DWShopProject.order.repository.AOrderItemRepository;
import com.example.DWShopProject.order.repository.AOrderRepository;
...
public class AOrderService {
    ...
    // 모든 주문 상태를 가져오는 메서드
    @Transactional(readOnly = true)
    public List<OrderStatusEnum> getAllOrderStatuses() {
    ...
    public AOrderDto updateOrderStatus(Long id, String status) {
        ...
        order.updateOrderInfo(null, null, null, null, -1, OrderStatusEnum.valueOf(status));
    ...
    public List<AOrderDto> updateOrderStatuses(List<Long> ids, String status) {
        ...
        orders.forEach(order -> order.updateOrderInfo(null, null, null, null, -1, OrderStatusEnum.valueOf(status)));
