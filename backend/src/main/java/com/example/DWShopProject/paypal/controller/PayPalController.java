package com.example.DWShopProject.paypal.controller;
...
import com.example.DWShopProject.order.repository.TemporaryOrderRepository;
...
public class PayPalController {
    ...
    public ResponseEntity<?> createPayment(@AuthenticationPrincipal MemberDetailsImpl memberDetails, @RequestBody OrderDto orderDto) {
        ...
        String finalReturnUrl = "http://1.226.84.83:8080/api/paypal/success?tempOrderId=" + temporaryOrder.getId();
    ...
    public void success(@RequestParam("paymentId") String paymentId, @RequestParam("PayerID") String payerId, @RequestParam("tempOrderId") Long tempOrderId, HttpServletResponse response) throws IOException {
        ...
        if (captureResponse != null) {
            ...
            String redirectUrl = "http://localhost:3500/order/detail/" + order.getId();
            ...
            response.sendRedirect("http://localhost:3500/mypage?error=Error+capturing+payment");
        }
    }

    @GetMapping("/cancel")
    public void paymentCancel(HttpServletResponse response) throws IOException {
        response.sendRedirect("http://localhost:3500/mypage?error=Payment+was+cancelled");
    }
